...
void OnNotification(Notification const* _notification, void* _context) {
    ...
    switch (_notification->GetType()) {
        ...
        case Notification::Type_ValueChanged:
        {
			...
			uint8 val; uint8* pVal = &val;

			Log::Write(LogLevel_Info,"-------------------Bool: %d Byte: %d Dec: %d Int: %d List: %d Short: %d Str: %d",
				    ValueID::ValueType_Bool, ValueID::ValueType_Byte, ValueID::ValueType_Decimal, ValueID::ValueType_Int,
				    ValueID::ValueType_List, ValueID::ValueType_Short, ValueID::ValueType_String);
			
			ValueID w = _notification->GetValueID();
			Log::Write(LogLevel_Info, "------------------- LEVEL TYPE: %lu", w.GetType());
			
			
			
			bool ret = Manager::Get()->GetValueAsByte(w, pVal);
				if(ret)
				{
				    nodeInfo->m_value = pVal[0];
				    Log::Write(LogLevel_Info,"-----------------------VALUE: %d", nodeInfo->m_value);
				    nodeInfo->m_notapplicable = false;
				}
				else
				{
				    Log::Write(LogLevel_Info,"-----------------------GET VALUE FAILED!\n");
				}
...
int main(int argc, char* argv[]) {
    ...
    Options::Create("conf/zwave/", "zwave/", "");
    Options::Get()->Lock();
    ...
    if (!g_initFailed) {
        ...
        try {
			...
			if (trim(data.c_str()) == "VALUES") {
			    string values;
                            for (list<NodeInfo*>::iterator it = g_nodes.begin(); it != g_nodes.end(); ++it) {
                                NodeInfo* nodeInfo = *it;
                                int nodeID = nodeInfo->m_nodeId;
                                
				stringstream ssID, ssVal;
                                    				    
				    if(nodeInfo->m_notapplicable)
				    {
					ssVal << "NONE";
				    }
				    else
				    {
					ssVal << (int)nodeInfo->m_value;
                                    }

				    ssID << nodeID;

                                    values += "VALUES~" + ssID.str() + "~" + ssVal.str() + "#";
                            }
    
                            values = values.substr(0, values.size() - 1) + "\n";                           
                            printf("Sent Values List \n");
                            new_sock << values;
                        }
			...
			///////////////////////////////////////
				...
				string nodeProduct = Manager::Get()->GetNodeProductName(g_homeId, nodeInfo->m_nodeId);
				
				//int len = sizeof(nodeInfo->m_value);
				//std::string valu( nodeInfo->m_value, len );
;				
                                if (nodeName.size() == 0) nodeName = "Undefined";

                                //if (nodeType != "Static PC Controller") {
                                    stringstream ssNodeName, ssValue, ssNodeId, ssNodeType, ssNodeZone, ssNodeProduct;
                                    ssNodeName << nodeName;
				    ...
				    {
					ssValue << (int)nodeInfo->m_value;
                                    }

				    ssNodeId << nodeID;
                                    ssNodeType << nodeType;
                                    ssNodeZone << nodeZone;
				    ssNodeProduct << nodeProduct;

                                    device += "DEVICE~" + ssNodeName.str() + "~" + ssNodeId.str() + "~"+ ssNodeZone.str() +"~" + ssNodeType.str() + "~" + ssNodeProduct.str() + "~" + ssValue.str() + "#";
                                //}
                            }
